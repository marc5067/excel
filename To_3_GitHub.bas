    ...
    For Each vbComp In ThisWorkbook.VBProject.VBComponents
        Select Case vbComp.Type
            ...
            Case vbext_ct_MSForm
                ...
                ' Create an associated .frx file if it exists
                filePath = repoPath & vbComp.Name & ".frx"
                If FileExists(vbComp, ".frx") Then
                    ExportFRXFile vbComp, filePath
                Else
                    ' Create an empty .frx file as a placeholder
                    fileNum = FreeFile
...
' Check if the .frx file exists for a given component
Function FileExists(vbComp As VBComponent, ext As String) As Boolean
    On Error Resume Next
    FileExists = (Dir(vbComp.Name & ext) <> "")
    On Error GoTo 0
End Function

' Export the .frx file associated with a form if it exists
Sub ExportFRXFile(vbComp As VBComponent, filePath As String)
    Dim fileNum As Integer
    fileNum = FreeFile
    Open filePath For Binary Access Write As #fileNum
    Put #fileNum, 1, vbComp.Designer.OleObjectBlob
    Close #fileNum
End Sub
